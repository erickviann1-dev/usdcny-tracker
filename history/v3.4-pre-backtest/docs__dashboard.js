...
const I18N = {
  en: {
    ...
    "backtest.title":    "Verdict signal · cumulative track record",
    ...
    "flip.col.var":      "Input",
    "flip.col.today":    "Today",
    "flip.col.yes":      "YES boundary",
    "flip.col.no":       "NO boundary",
...
function renderAll(data) {
    ...
    // v3.4 — decision layer + v3.5 backtest / flip lines
    renderVerdict(data.decision && data.decision.carry_verdict);
    renderVerdictBacktest(data.backtest);
    renderFlipLines(data.flip_lines);
...
function renderVerdictBacktest(bt) {
    const wrap = document.getElementById("verdict-backtest-wrap");
    const statsEl = document.getElementById("verdict-backtest-stats");
    if (!wrap || !statsEl) return;
    if (!bt || !bt.dates || bt.dates.length < 10) {
    ...
    const cum = bt.cumulative || [];
    const bcum = bt.benchmark_cumulative || [];
    const scale = arr => {
        const base = arr[0] || 1;
    ...
    Plotly.newPlot("chart-verdict-backtest", [
        { x: dates, y: yS, name: t("backtest.strategy"), type: "scatter",
          line: { color: COLORS.warn, width: 2 }, hovertemplate: "%{y:.3f}<extra></extra>" },
        { x: dates, y: yB, name: t("backtest.benchmark"), type: "scatter",
          line: { color: COLORS.muted, width: 1.5, dash: "dot" },
          hovertemplate: "%{y:.3f}<extra></extra>" },
    ], {
        ...LAYOUT_BASE,
        margin: { l: 44, r: 10, t: 24, b: 40 },
        height: 220,
        showlegend: true,
        title: { text: t("backtest.title"), font: { size: 11, color: COLORS.muted } },
        xaxis: { ...LAYOUT_BASE.xaxis, tickangle: -35, ntick: 10 },
        yaxis: { ...LAYOUT_BASE.yaxis,
                 title: { text: t("backtest.axis"), font: { size: 10 } },
                 tickformat: ".1f" },
...
function renderFlipLines(rows) {
    const wrap = document.getElementById("verdict-flip-wrap");
    const head = document.getElementById("verdict-flip-head");
    const grid = document.getElementById("verdict-flip-lines");
    ...
    wrap.style.display = "";
    head.textContent = t("flip.title");

    const rateInputs = new Set(["us_1y", "shibor_1y", "cnh_hibor_1y"]);
    const isRate = inp => rateInputs.has(inp);
    const fmtNum = (v, inp) => {
        if (v == null || v === undefined) return "—";
        return isRate(inp) ? `${Number(v).toFixed(2)}%` : Number(v).toFixed(4);
    };
    const fmtDistYes = row => {
        if (isRate(row.input)) {
            if (row.dist_yes_pp == null) return "—";
            const bps = Math.round(row.dist_yes_pp * 100);
            return `${bps >= 0 ? "+" : ""}${bps} bp`;
        }
        if (row.dist_yes_pct == null) return "—";
        const p = row.dist_yes_pct;
        return `${p >= 0 ? "+" : ""}${p.toFixed(1)}%`;
    };
    const fmtDistNo = row => {
        if (isRate(row.input)) {
            if (row.dist_no_pp == null) return "—";
            const bps = Math.round(row.dist_no_pp * 100);
            return `${bps >= 0 ? "+" : ""}${bps} bp`;
        }
        if (row.dist_no_pct == null) return "—";
        const p = row.dist_no_pct;
        return `${p >= 0 ? "+" : ""}${p.toFixed(1)}%`;
    };

    let html = `<div class="flip-row flip-header"><span>${t("flip.col.var")}</span>`
        + `<span>${t("flip.col.today")}</span>`
        + `<span>${t("flip.col.yes")}</span>`
        + `<span>${t("flip.col.no")}</span></div>`;

    rows.slice(0, 6).forEach(row => {
        const lab = LANG === "zh" ? row.label_zh : row.label_en;
        const yesCell = row.flip_to_yes != null
            ? `${fmtNum(row.flip_to_yes, row.input)} (${fmtDistYes(row)})`
            : "—";
        const noCell = row.flip_to_no != null
            ? `${fmtNum(row.flip_to_no, row.input)} (${fmtDistNo(row)})`
            : "—";
        html += `<div class="flip-row">
            <span class="flip-var">${escapeHtml(lab)}</span>
            <span>${fmtNum(row.today, row.input)}</span>
            <span>${yesCell}</span>
            <span>${noCell}</span>
        </div>`;
    });
